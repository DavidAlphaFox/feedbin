  ...
  class IndexView < ApplicationView
    ...
    def template
      form_for @user, remote: true, url: helpers.settings_update_user_path(@user) do |f|
        ...
        render Settings::ControlGroup.new(class: "mb-14") do |group|
          group.header do
            "Unread Sort"
          end
          group.item do
            ...
            f.label :entry_sort_desc, class: "group" do
              render Settings::ControlRow.new do |row|
                row.title { "Newest First" }
                row.control { render Form::Radio.new }
              end
            end
          end
          group.item do
        ...
        render Settings::ControlGroup.new(class: "mb-14") do |group|
          ...
          group.item do
            ...
            f.label :hide_updated, class: "group" do
              render Settings::ControlRow.new do |row|
                row.title { "Updated" }
                row.control { render Form::Switch.new }
              end
            end
          end
          group.item do
