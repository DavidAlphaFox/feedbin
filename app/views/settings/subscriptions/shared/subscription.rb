                    ...
                    span class: "truncate" do
                      span class: "block truncate" do
                        ...
                        span class: "text-500 text-sm" do
                          text helpers.timeago(@subscription.last_published_entry)
                          text ", #{helpers.number_with_delimiter(@subscription.post_volume)}/mo"
                        end
                      end
                      span class: "block truncate !text-500 text-sm" do
                        text helpers.short_url(@subscription.feed_url)
                      end
                    end
                    span class: "ml-auto flex items-center gap-4" do
                      render Components::Svg.new "menu-icon-mute", class: helpers.class_names(["fill-600", {"tw-hidden" => !@subscription.muted?}]), title: "Muted", data: {toggle: "tooltip"}
                      ...
                      render Components::Svg.new "icon-caret", class: "fill-300 -rotate-90"
                    end
                  end
                end
              end
